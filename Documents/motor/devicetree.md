...
电机节点通过 `controllers` 引用一个或多个 `motor-controller,*` 节点。每个控制器节点自己声明模式、目标和参数，不再有电机 `capabilities`，电机控制器也不再引用旧的通用控制器设备。
...
`VO + MOTOR_TARGET_TORQUE` 可以是驱动原生直通扭矩目标，不一定需要控制器节点。例如 DJI 的直接扭矩控制就是 `motor_set_torque()` 路径。
...
PV 控制器会优先读取 `pos_*`、`vel_*` 的分环参数；未配置时回退到通用 `i_max`、`out_max`、`detri_lpf`、`offset`。
...
- `rx_id`: 电机接收 ID
- `can_channel`: CAN 总线节点
- `gear_ratio`: 减速比，按驱动支持情况配置
- `controllers`: 控制器节点列表，可选
- `p_max` / `v_max` / `t_max`: 角度、速度、扭矩限制，按驱动支持情况配置
- `read_only`: DM 电机可选。配置后只接收反馈，不自动发送使能或控制帧。

DM 控制请求频率由 `CONFIG_MOTOR_DM_DEFAULT_FREQ_HZ` 配置，默认 `200 Hz`。节点内不再声明 `freq` 属性。
...
VESC 支持原生 `VO + TORQUE`、`VO + SPEED` 和 `PV + POSITION` 下发路径，可以不配置 `controllers`。只有需要用户态枚举并显式选择控制器时，才需要声明控制器节点。
