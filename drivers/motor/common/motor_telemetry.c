...
#ifndef CONFIG_MOTOR_TELEMETRY_CAN_BUSY_WINDOW_US
#define CONFIG_MOTOR_TELEMETRY_CAN_BUSY_WINDOW_US 800
#endif
					  ...
					  const struct motor_can_sched_stats *stats)
{
	...
	pressure = (tx_busy_delta != 0U) || (dropped_delta != 0U) || (ack_timeout_delta != 0U) ||
		   ...
		   (busy_window_us >= CONFIG_MOTOR_TELEMETRY_CAN_BUSY_WINDOW_US);
	if (!pressure) {
		return;
	}
	...
	LOG_WRN_RATELIMIT_RATE(CONFIG_MOTOR_TELEMETRY_CAN_LOG_INTERVAL_MS,
			       ...
			       "pending_full+%u giveup+%u window=%uus(tx=%u rx=%u reserved=%u)",
			       can_dev->name, tx_busy_delta, dropped_delta, ack_timeout_delta,
			       pending_full_delta, giveup_delta, busy_window_us,
